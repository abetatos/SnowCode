...
class DenseUnet(nn.Module):
    def __init__(self, in_channels, out_channels, growth_rate=32, block_config=(6, 12, 24, 16), bn_size=4, drop_rate=0, downsample=False, pretrained_encoder_uri=None, progress=None, **kwargs):
        super(DenseUnet, self).__init__()
        self.skip_connections = []
        self.encoder = _DenseUNetEncoder(self.skip_connections, growth_rate, block_config, in_channels, bn_size, drop_rate, downsample)
        self.decoder = _DenseUNetDecoder(self.skip_connections, growth_rate, block_config, in_channels, bn_size, drop_rate, downsample)
        self.classifier = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, bias=True)
